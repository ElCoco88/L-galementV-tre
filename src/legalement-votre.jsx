...
// La clé API est stockée UNIQUEMENT sur l'appareil de l'utilisateur.
// Le créateur du site ne reçoit aucune clé, aucun argent via l'API.
// La seule rémunération du créateur provient de publicités discrètes.
// ============================================================

function getApiKey() {
  try { return localStorage.getItem('lv_api_key') || ''; } catch(_e) { return ''; }
}
function saveApiKey(k) {
  try { localStorage.setItem('lv_api_key', k.trim()); } catch(_e) {}
}
function clearApiKey() {
  try { localStorage.removeItem('lv_api_key'); } catch(_e) {}
}

// Injecte la clé dans les headers de chaque appel API
...
// Écran de configuration de la clé API
function ApiKeyScreen({ onSave }) {
  ...
  return (
    <div style={{ padding: '16px 16px 80px' }}>
      {/* Bannière principale */}
      <div style={{ background: 'linear-gradient(135deg, rgba(37,99,235,0.08), rgba(124,58,237,0.06))', border: '1px solid rgba(37,99,235,0.2)', borderRadius: 16, padding: '20px', marginBottom: 16 }}>
        <div style={{ fontSize: 32, marginBottom: 12, textAlign: 'center' }}>⚖️</div>
        <h2 style={{ fontWeight: 900, color: 'var(--text-primary)', fontSize: 18, marginBottom: 8, textAlign: 'center' }}>
          LégalementVôtre est 100% gratuit
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.6, marginBottom: 0, textAlign: 'center' }}>
          Les lettres, simulateurs, glossaire, délais et analyse automatique sont gratuits sans aucune configuration.
        </p>
      </div>

      {/* Explication transparente */}
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px', marginBottom: 16 }}>
        <h3 style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: 15, marginBottom: 12 }}>
          💬 Pourquoi certaines fonctions demandent une clé ?
        </h3>
        <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.7, marginBottom: 10 }}>
          Les fonctions qui utilisent l'intelligence artificielle (Chat IA, analyse approfondie des fiches, génération de lettres sur mesure) nécessitent des serveurs coûteux.
        </p>
        <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.7, marginBottom: 10 }}>
          Pour que le site reste 100% gratuit, chaque utilisateur peut créer son propre compte chez Anthropic (la société qui développe Claude) et utiliser ses propres crédits directement.
        </p>
        <div style={{ background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(22,163,74,0.25)', borderRadius: 10, padding: '12px', marginBottom: 10 }}>
          <p style={{ color: '#16a34a', fontSize: 13, fontWeight: 700, margin: 0 }}>
            ✅ Le créateur de ce site ne perçoit aucun argent via les clés API.
          </p>
          <p style={{ color: 'var(--text-secondary)', fontSize: 12, margin: '4px 0 0', lineHeight: 1.5 }}>
            Si vous utilisez l'IA, votre paiement va directement à Anthropic — pas au créateur du site. Ce site est avant tout un outil militant pour aider les salariés.
          </p>
        </div>
        <div style={{ background: 'rgba(234,179,8,0.08)', border: '1px solid rgba(234,179,8,0.25)', borderRadius: 10, padding: '12px', marginBottom: 10 }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 12, margin: '4px 0 0', lineHeight: 1.5 }}>
            Le site affiche quelques publicités discrètes. C'est la seule façon pour le créateur d'être légèrement rémunéré pour le temps consacré à ce projet. Le choix a été fait de limiter au maximum les publicités pour ne pas nuire à votre expérience. Elles ne sont jamais placées dans les pages d'analyse ou de résultats.
          </p>
        </div>
        <div style={{ background: 'rgba(37,99,235,0.08)', border: '1px solid rgba(37,99,235,0.2)', borderRadius: 10, padding: '12px' }}>
          <p style={{ color: '#2563eb', fontSize: 13, fontWeight: 700, margin: '0 0 4px' }}>
            🎁 Anthropic offre des crédits gratuits à l'inscription
          </p>
          <p style={{ color: 'var(--text-secondary)', fontSize: 12, margin: 0, lineHeight: 1.5 }}>
            Quand vous créez un compte sur console.anthropic.com, vous recevez des crédits gratuits. Pour la majorité des utilisateurs, ces crédits suffisent largement.
          </p>
        </div>
      </div>
      ...
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px', marginBottom: 16 }}>
        <h3 style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: 15, marginBottom: 14 }}>
          📋 Comment obtenir votre clé gratuite
        </h3>
        {[
          { n: '1', text: 'Allez sur console.anthropic.com et créez un compte gratuit', link: 'https://console.anthropic.com' },
          { n: '2', text: 'Dans le menu, allez dans "API Keys"' },
          { n: '3', text: 'Cliquez "Create Key", donnez-lui un nom (ex: LégalementVôtre)' },
          { n: '4', text: 'Copiez la clé (commence par sk-ant-...)' },
          { n: '5', text: 'Collez-la ci-dessous — elle est sauvegardée uniquement sur votre appareil' },
        ].map(s => (
          <div key={s.n} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 12 }}>
            <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'linear-gradient(135deg,#2563eb,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 800, flexShrink: 0, marginTop: 1 }}>{s.n}</div>
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
              {s.text}
              {s.link && <><br/><a href={s.link} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', fontSize: 12 }}>{s.link}</a></>}
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Champ de saisie */}
      <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px', marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: 0.5, display: 'block', marginBottom: 8 }}>VOTRE CLÉ API ANTHROPIC</label>
        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          <input
            ...
            placeholder="sk-ant-..."
            style={{ flex: 1, background: 'var(--input-bg)', border: '1.5px solid var(--border)', borderRadius: 10, padding: '10px 12px', color: 'var(--text-primary)', fontSize: 13, outline: 'none' }}
          />
          <button onClick={() => setShowKey(!showKey)} style={{ background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 10, padding: '0 12px', cursor: 'pointer', fontSize: 16, color: 'var(--text-muted)' }}>
            {showKey ? '🙈' : '👁'}
          </button>
        </div>
        <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '0 0 12px', lineHeight: 1.5 }}>
          🔒 Votre clé est sauvegardée uniquement sur cet appareil. Elle ne passe jamais par nos serveurs.
        </p>
        <button
          onClick={() => { if (key.startsWith('sk-ant-')) { saveApiKey(key); onSave(key); } }}
          disabled={!key.startsWith('sk-ant-')}
          style={{ width: '100%', background: key.startsWith('sk-ant-') ? 'linear-gradient(135deg,#2563eb,#7c3aed)' : 'var(--input-bg)', color: key.startsWith('sk-ant-') ? '#fff' : 'var(--text-muted)', border: 'none', borderRadius: 12, padding: '14px', fontWeight: 800, fontSize: 15, cursor: key.startsWith('sk-ant-') ? 'pointer' : 'not-allowed' }}
        >
      ...
      <p style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', lineHeight: 1.6 }}>
        Vous pouvez utiliser toutes les fonctions gratuites sans clé.<br/>La clé n'est nécessaire que pour le Chat IA et l'analyse approfondie.
      </p>
    </div>
  );
}
...
function RightsNavigatorPage({ setPage }) {
  ...
  async function getDetail(topic) {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: buildHeaders(getApiKey()),
        body: JSON.stringify({
...
// ════════════════════════════════════════════════════════════════
// ============================================================
// MOTEUR D'ANALYSE PAR RÈGLES (100% gratuit, sans IA)
// ============================================================

const SMIC_HORAIRE = 11.88; // 2025
const SMIC_MENSUEL_35H = 1801.80;

function analyzeWithRules(ficheText, contratText, manualCC, CC_BASE) {
  const text = ficheText.toLowerCase();
  const anomalies = [];
  const indemnites = [];
  const violations = [];
  ...
  const salaireBrut = extractNumber([
    /salaire\s*brut[^\d]*(\d[\d\s,.']*)/i,
    /brut\s*mensuel[^\d]*(\d[\d\s,.']*)/i,
    /total\s*brut[^\d]*(\d[\d\s,.']*)/i,
  ]);

  const salaireNet = extractNumber([
    /net\s*[àa]\s*payer[^\d]*(\d[\d\s,.']*)/i,
    /net\s*imposable[^\d]*(\d[\d\s,.']*)/i,
    /salaire\s*net[^\d]*(\d[\d\s,.']*)/i,
  ]);

  const heures = extractNumber([
    /heures?\s*(?:travaillées?|effectuées?)[^\d]*(\d[\d,.]*)/i,
    /nb\s*heures?[^\d]*(\d[\d,.]*)/i,
    /(\d[\d,.]*)\s*h(?:eures?)?\s*(?:de\s*)?(?:travail|base)/i,
  ]);

  const heuresSup = extractNumber([
    /heures?\s*supp[^\d]*(\d[\d,.]*)/i,
    /h\.?s\.?[^\d]*(\d[\d,.]*)/i,
  ]);

  // ── Vérification SMIC ──────────────────────────────────────────
  if (salaireBrut && heures && heures > 0) {
    const tauxHoraire = salaireBrut / heures;
    if (tauxHoraire < SMIC_HORAIRE) {
      anomalies.push({
        ligne: "Salaire brut",
        probleme: `Taux horaire (${tauxHoraire.toFixed(2)}€/h) inférieur au SMIC (${SMIC_HORAIRE}€/h)`,
        impact: `${((SMIC_HORAIRE - tauxHoraire) * heures).toFixed(2)}€`,
        gravite: "haute",
        article: "L3231-2 Code du travail",
      });
    }
  } else if (salaireBrut && salaireBrut < SMIC_MENSUEL_35H && !text.includes("temps partiel") && !text.includes("mi-temps")) {
    anomalies.push({
      ligne: "Salaire brut",
      probleme: `Salaire brut (${salaireBrut}€) inférieur au SMIC mensuel 35h (${SMIC_MENSUEL_35H}€)`,
      impact: `${(SMIC_MENSUEL_35H - salaireBrut).toFixed(2)}€`,
      gravite: "haute",
      article: "L3231-2 Code du travail",
    });
  }

  // ── Vérification heures supplémentaires ───────────────────────
  if (heuresSup && heuresSup > 0) {
    const hasHSMajoration = text.includes('25%') || text.includes("majorat") || text.includes("h.s.") || text.includes("hrs maj");
    if (!hasHSMajoration) {
      anomalies.push({
        ligne: "Heures supplémentaires",
        probleme: `${heuresSup}h sup détectées sans majoration apparente (25% obligatoire pour les 8 premières)`,
        impact: salaireBrut ? `~${((salaireBrut / (heures||151.67)) * heuresSup * 0.25).toFixed(2)}€` : '?',
        gravite: "haute",
        article: "L3121-36 Code du travail",
      });
    }
  }

  // ── Vérification congés payés ──────────────────────────────────
  const hasConges = text.includes("congé") || text.includes('cp ') || text.includes("c.p.") || text.includes('indemnité de congé');
  if (!hasConges && salaireBrut) {
    indemnites.push({
      indemnite: "Indemnité de congés payés",
      montant_estime: `${(salaireBrut * 0.10).toFixed(2)}€`,
      base_legale: "L3141-24 — 10% du brut minimum",
      gravite: "moyenne",
    });
  }

  // ── Vérification cotisations obligatoires ─────────────────────
  const cotisationsRequises = [
    { nom: "Sécurité sociale", patterns: ["sécu", "sécurité sociale", "maladie", "vieillesse"] },
    { nom: "Retraite complémentaire", patterns: ["retraite", "agirc", "arrco"] },
    { nom: "CSG/CRDS", patterns: ["csg", "crds"] },
    { nom: "Assurance chômage", patterns: ["chômage", "pole emploi", "pôle emploi", "unédic"] },
  ];

  for (const cot of cotisationsRequises) {
    const found = cot.patterns.some(p => text.includes(p));
    if (!found) {
      anomalies.push({
        ligne: "Cotisations",
        probleme: `Cotisation "${cot.nom}" absente ou non identifiable`,
        impact: 'À vérifier',
        gravite: "moyenne",
  ...
  // ── Vérification prime ancienneté selon CC ────────────────────
  const cc = manualCC || null;
  if (cc) {
    const ancienneteM = extractNumber([/ancienneté[^\d]*(\d+)\s*mois/i, /depuis[^\d]*(\d+)\s*mois/i]);
    if (ancienneteM && ancienneteM >= 36) {
      const hasPrimeAnc = text.includes("ancienneté") || text.includes("prime anc");
      if (!hasPrimeAnc) {
        indemnites.push({
          indemnite: "Prime d'ancienneté",
          montant_estime: salaireBrut ? `~${(salaireBrut * 0.03).toFixed(2)}€ (3% min)` : 'À calculer',
          base_legale: `Convention collective ${cc.nom} — ancienneté ≥ 3 ans`,
          gravite: "moyenne",
        });
      }
    }
  ...
  // ── Vérification bulletin conforme L3243-1 ────────────────────
  const mentionsObligatoires = [
    { label: "Nom employeur", patterns: ["employeur", "société", "sas", "sarl", "siren", "siret"] },
    { label: "Convention collective", patterns: ["convention", "cc ", "idcc"] },
    { label: "Période de paie", patterns: ["période", "mois", "janvier", "février", "mars", "avril", "mai", "juin", "juillet", "août", "septembre", "octobre", "novembre", "décembre"] },
  ];

  for (const m of mentionsObligatoires) {
    if (!m.patterns.some(p => text.includes(p))) {
      violations.push({
        type: "Mention obligatoire",
        violation: `"${m.label}" absent du bulletin`,
  ...
  // ── Résumé ─────────────────────────────────────────────────────
  const score = anomalies.filter(a => a.gravite === 'haute').length > 0 ? "alerte"
    : anomalies.length + indemnites.length > 0 ? "attention" : "ok";

  return {
    methode: "regles",
    salaire_brut: salaireBrut ? salaireBrut + '€' : null,
    salaire_net: salaireNet ? salaireNet + '€' : null,
    heures_travaillees: heures ? heures + 'h' : null,
    anomalies,
    indemnites_manquantes: indemnites,
    violations_legales: violations,
    score,
    conseil: score === "alerte"
      ? "Des anomalies importantes ont été détectées. Vérifiez les points signalés en rouge."
      : score === "attention"
      ? "Quelques points méritent vérification. Comparez avec votre contrat."
      : "Aucune anomalie évidente détectée par l'analyse automatique.",
  };
}

// ── Bannière transparence ─────────────────────────────────────────
function TransparenceBanner() {
  const [open, setOpen] = useState(false);
  return (
    <div style={{ background: 'rgba(37,99,235,0.07)', border: '1px solid rgba(37,99,235,0.2)', borderRadius: 12, padding: '10px 14px', marginBottom: 16, fontSize: 13 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }} onClick={() => setOpen(!open)}>
        <span>💙 <strong>LégalementVôtre est 100% gratuit</strong> — comment c'est possible ?</span>
        <span style={{ color: 'var(--text-muted)', fontSize: 11 }}>{open ? '▲' : '▼'}</span>
      </div>
      {open && (
        <div style={{ marginTop: 10, color: 'var(--text-secondary)', lineHeight: 1.6, fontSize: 12 }}>
          <p style={{ marginBottom: 8 }}>Toutes les fonctionnalités de base — lettres, simulateurs, glossaire, délais, journal des preuves, analyse automatique — fonctionnent <strong>sans IA et sans inscription</strong>. Elles sont 100% gratuites.</p>
          <p style={{ marginBottom: 8 }}>Les fonctions IA (chat juridique, analyse approfondie) utilisent votre propre clé API Anthropic. <strong>Le créateur du site ne reçoit rien</strong> — l'argent va directement à Anthropic.</p>
          <p style={{ marginBottom: 8 }}>Le site affiche quelques publicités discrètes. C'est la seule rémunération du créateur, qui a volontairement choisi d'en mettre le moins possible.</p>
          <p style={{ color: '#2563eb', fontWeight: 600 }}>Engagement : la majorité des outils resteront toujours gratuits et sans pub intrusive.</p>
        </div>
      )}
    </div>
  );
...
function MultiFichesAnalyzer({ setPage, PROMPTS, safeParseJSON, sanitizeFicheResult }) {
  ...
  const [useAI, setUseAI] = useState(false); // false = règles gratuites, true = IA payante
...
FORMAT EXACT:{"violations_legales":[{"type":"Loi ou CC","article":"L1234-5","violation":"description courte","sanction":"sanction applicable","impact":"Xe ou null"}]}`;
  ...
  async function analyzeAll() {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        // ── Vérification clé API si mode IA ──────────────────────────
        if (useAI && !getApiKey()) {
          setAnalyzing(false);
          alert('Ajoutez votre clé API pour utiliser le mode IA. Appuyez sur "🔑 Config IA" en haut.');
          return;
        }

        // ── Mode gratuit (règles) ───────────────────────────────────
        if (!useAI) {
          const rulesResult = analyzeWithRules(fiche.text, contrats.map(c=>c.text).join('\n'), manualCC, CC_BASE);
          const idx2 = results.findIndex(f => f.id === fiche.id);
          results[idx2] = {
            ...fiche,
            status: 'done',
            data: {
              mois_annee: null,
              salaire_brut: rulesResult.salaire_brut,
              salaire_net: rulesResult.salaire_net,
              heures_travaillees: rulesResult.heures_travaillees,
              anomalies: rulesResult.anomalies,
              indemnites_manquantes: rulesResult.indemnites_manquantes,
              violations_legales: rulesResult.violations_legales,
              conseil: rulesResult.conseil,
              methode: "regles",
            }
          };
          setFiches([...results]);
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12 }}>
          <div>
            ...
            <p style={{ color:'var(--text-muted)', fontSize:12, margin:0, lineHeight:1.5 }}>
              {useAI ? "L'IA analyse chaque fiche en détail (coût API)." : "Analyse automatique gratuite — détecte SMIC, heures sup, cotisations, congés payés."}
            </p>
            <div style={{ display:'flex', gap:6, marginTop:8, alignItems:'center' }}>
              <button onClick={()=>setUseAI(false)} style={{ fontSize:11, padding:'4px 10px', borderRadius:20, border:'none', cursor:'pointer', background: !useAI ? '#16a34a' : 'rgba(0,0,0,0.08)', color: !useAI ? '#fff' : 'var(--text-muted)', fontWeight:700 }}>✅ Gratuit</button>
              <button onClick={()=>setUseAI(true)} style={{ fontSize:11, padding:'4px 10px', borderRadius:20, border:'none', cursor:'pointer', background: useAI ? '#7c3aed' : 'rgba(0,0,0,0.08)', color: useAI ? '#fff' : 'var(--text-muted)', fontWeight:700 }}>🤖 IA (avancé)</button>
              <span style={{ fontSize:10, color:'var(--text-muted)' }}>{useAI ? '⚠ Nécessite clé API' : '🆓 Sans IA'}</span>
            </div>
          </div>
...
export default function App() {
  ...
  function renderPage() {
    switch(page) {
      ...
      case 'apikey': return <ApiKeyScreen onSave={k => { setPage('home'); }} />;
  ...
  return (
    <div className={`theme-${theme}`} style={{ minHeight:'100vh', background:'var(--bg)', color:'var(--text-primary)', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif", maxWidth:480, margin:'0 auto', position:'relative' }}>
      ...
      <div style={{ position:'sticky', top:0, zIndex:100, background:'var(--card-bg)', borderBottom:'1px solid var(--border)', padding:'10px 16px', display:'flex', alignItems:'center', gap:10 }}>
        ...
        <button onClick={()=>setPage('apikey')} title="Configurer ma clé API" style={{ background: getApiKey() ? 'rgba(22,163,74,0.15)' : 'rgba(239,68,68,0.1)', border: '1px solid ' + (getApiKey() ? 'rgba(22,163,74,0.3)' : 'rgba(239,68,68,0.2)'), borderRadius: 20, padding: '3px 8px', fontSize: 10, fontWeight: 700, color: getApiKey() ? '#16a34a' : '#ef4444', cursor: 'pointer', whiteSpace: 'nowrap' }}>
          {getApiKey() ? '🔑 IA active' : '🔑 Config IA'}
        </button>
